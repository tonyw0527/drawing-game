...
let invitation_code = process.env.INVITATION_CODE;
...
io.on("connection", (socket: any) => {
  ...
  socket.on("login", ({ myName, code }: Login) => {
    ...
    if (users.findIndex((user: any) => user.myName === loginUserName) > -1) {
  ...
  // game logic
  socket.on("book host", (MyName: any) => {
    gameManager.handleBooking(MyName);
  });

  socket.on("send answer", (answerFromHost: any) => {
  ...
  socket.on("send data", (data: any) => {
    socket.broadcast.emit("send data", data);
  });

  socket.on("cashing image", ({ cashingImage, type }: any) => {
  ...
  // db logic
  socket.on("db-create", ({ title, content }: any) => {
    const flag = mongoManager.create(title, content);
    console.log(flag);
  });

  // chat logic
  socket.on("send msg", ({ MyName, Message }: any) => {
    ...
    if (isAdminLogined && MyName === adminName) {
      ...
      if (cmd === "code") {
        ...
        const users = gameManager.getUsers();
        const flag = users.findIndex((user: any) => user.myName === arg1);
        if (flag === -1) {
          socket.emit("send msg", {
            name: "info-logout",
  ...
  // gallaery
  socket.on("gallery-login", ({ nickName, code }: any) => {
  ...
  socket.on("db-update", ({ id, name }: any) => {
    console.log("db-update", id, name);
    mongoManager.update(socket, id, name);
  });

  socket.on("db-delete", (id: any) => {
